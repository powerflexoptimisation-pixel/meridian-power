...
export async function getNodeTimeseries({ nodeKeys, seriesTypes, resolution, from, to }) {
  ...
  for (const key of nodeKeys) {
    ...
    // Étape 3: bucketing selon la résolution choisie.
    // - Sur-échantillonnage (pas demandé >= pas brut): agrégation classique,
    //   moyenne pour la puissance, somme d'énergie (v * stepHours) pour l'énergie.
    // - Sous-échantillonnage (pas demandé < pas brut, ex: 15min demandé alors que
    //   la donnée est horaire): on ne laisse pas de trous. La puissance reste
    //   constante sur chaque sous-pas (c'est la même mesure physique répétée);
    //   l'énergie du pas brut est répartie au prorata (divisée) entre les sous-pas.
    const rawStepMs = stepHours * 3_600_000;
    for (const seriesType of seriesTypes) {
      const buckets = new Map(); // bucketIso -> { sum, count }
      for (const tsIso of sortedTs) {
        const v = byTsSeries.get(tsIso)[seriesType];
        if (v === undefined) continue;
        if (res.unitMs && res.unitMs < rawStepMs) {
          const tsMs = new Date(tsIso).getTime();
          const nSub = Math.round(rawStepMs / res.unitMs);
          for (let subMs = tsMs; subMs < tsMs + rawStepMs; subMs += res.unitMs) {
            const bIso = bucketStart(new Date(subMs), resolution).toISOString();
            if (!buckets.has(bIso)) buckets.set(bIso, { sum: 0, count: 0 });
            const b = buckets.get(bIso);
            b.sum += res.kind === "energy" ? (v * stepHours) / nSub : v;
            b.count += 1;
          }
        } else {
