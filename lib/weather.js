...
export async function fetchWeatherForecast(fuel, days = 7) {
  ...
  const url = `https://api.open-meteo.com/v1/dwd-icon?latitude=${lat}&longitude=${lon}&hourly=${vars}&forecast_days=${days}&timezone=UTC`;
...
export async function fetchWeatherHistorical(fuel, startDate, endDate) {
  ...
  const { lat, lon } = pointsToParams(points);
  const vars = fuel === "Solar" ? "shortwave_radiation,temperature_2m,cloud_cover" : "wind_speed_100m,wind_speed_10m";
  const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lon}&start_date=${startDate}&end_date=${endDate}&hourly=${vars}&timezone=UTC`;
  const res = await fetch(url);
