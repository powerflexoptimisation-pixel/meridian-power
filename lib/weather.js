...
// Nombre max de cellules de grille MaStR retenues (les plus fortement
// pondérées) — évite une URL/requête Open-Meteo démesurée si la grille
// compte beaucoup de cellules à faible capacité individuelle. Couvre en
// pratique l'essentiel de la capacité nationale (concentrée sur un nombre
// limité de zones à forte densité de parcs).
const MAX_GRID_POINTS = 60;

// Résout les points météo à utiliser pour une filière: grille MaStR
// (position réelle des parcs, agrégée) si elle a été chargée via
// /api/admin/backfill-mastr, sinon repli sur la pondération par capacité
// régionale (8 Länder, WEATHER_POINTS) — c'était le seul mode disponible
// avant l'intégration MaStR. Le solaire reste toujours sur WEATHER_POINTS
// (voir lib/mastr.js: ~5,7M d'unités solaires, hors de portée d'une
// collecte complète dans les contraintes Vercel).
async function resolvePoints(fuel) {
  if (fuel === "Solar") return WEATHER_POINTS[fuel];
  try {
    const grid = await getMastrGrid(fuel);
    if (grid.length > 0) return grid.slice(0, MAX_GRID_POINTS);
  } catch {
    // getMastrGrid indisponible (table pas encore créée, etc.) — repli silencieux.
  }
  return WEATHER_POINTS[fuel];
}
...
//
// shortwave_radiation_instant (pas shortwave_radiation) — bug trouvé lors
// d'un diagnostic de précision dégradée aux heures de lever/coucher de
// soleil (MAPE Solar 92,5% sur 31j): shortwave_radiation par défaut chez
// Open-Meteo est une MOYENNE de l'heure PRÉCÉDENTE (documenté: "la valeur
// à 6h00 est la moyenne entre 5h00 et 6h00, la valeur instantanée à 6h00
// est bien plus élevée"). Vérifié en direct: à 05h00, instant=17.5 W/m²
// vs moyenne=2.3 W/m² (7.6x plus élevé) — négligeable à midi (irradiance
// stable), mais dévastateur pendant les ramps rapides de lever/coucher,
// qui expliquaient l'essentiel du MAPE dégradé.
function varsFor(fuel, suffix = "") {
  const vars = fuel === "Solar" ? ["shortwave_radiation_instant", "temperature_2m"] : ["wind_speed_100m", "temperature_2m", "surface_pressure"];
  return vars.map((v) => v + suffix).join(",");
}
...
export async function fetchWeatherForecast(fuel, days = 7) {
  const points = await resolvePoints(fuel);
  ...
  return aggregateLocations(locations, fuel, points);
}
...
// Prévision "day-ahead-équivalente" HISTORIQUE — via la Previous Runs API
// d'Open-Meteo (previous-runs-api.open-meteo.com), qui archive la
// prévision TELLE QU'ELLE ÉTAIT ÉMISE à un horizon fixe (_previous_day1 =
// valeur prédite 24h avant l'échéance), et non la météo réellement
// observée après coup. Distinction importante: utiliser l'archive météo
// "réelle" (fetchWeatherHistorical) pour simuler un forecast passé
// tricherait — ça reviendrait à juger le modèle avec une information
// parfaite qu'il n'aurait jamais eue en production, gonflant
// artificiellement sa précision apparente. _previous_day1 a la même
// incertitude qu'un vrai forecast day-ahead, ce qui permet un backfill
// honnête pour comparer équitablement à ENTSO-E sur les 31 derniers jours
// sans attendre que l'historique s'accumule naturellement.
export async function fetchWeatherPreviousDay1(fuel, pastDays) {
  const points = await resolvePoints(fuel);
  const { lat, lon } = pointsToParams(points);
  const url = `https://previous-runs-api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=${varsFor(fuel, "_previous_day1")}&models=dwd_icon_d2&past_days=${pastDays}&forecast_days=1&timezone=UTC&wind_speed_unit=ms`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Open-Meteo previous-runs failed: ${res.status}`);
  const json = await res.json();
  const locations = Array.isArray(json) ? json : [json];
  return aggregateLocations(locations, fuel, points, "_previous_day1");
}
...
function aggregateLocations(locations, fuel, points, suffix = "") {
  const time = locations[0]?.hourly?.time || [];
  const radKey = "shortwave_radiation_instant" + suffix;
  const tempKey = "temperature_2m" + suffix;
  const windKey = "wind_speed_100m" + suffix;
  const pressureKey = "surface_pressure" + suffix;
  const out = [];
  for (let i = 0; i < time.length; i++) {
    ...
    for (let j = 0; j < locations.length; j++) {
      ...
      let f;
      if (fuel === "Solar") {
        const radiation = h[radKey]?.[i];
        const temp = h[tempKey]?.[i];
        if (radiation === undefined || radiation === null) continue;
        f = solarFeature(radiation, temp);
      } else {
        const v = h[windKey]?.[i];
        const temp = h[tempKey]?.[i];
        const pressure = h[pressureKey]?.[i];
