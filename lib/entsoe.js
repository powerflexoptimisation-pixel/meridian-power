...
export async function fetchCrossBorderFlow(fromCode, toCode, periodStart, periodEnd) {
  const domainOut = DOMAINS[fromCode] || fromCode;
  const domainIn = DOMAINS[toCode] || toCode;
  const xml = await fetchEntsoe({
    documentType: "A11",
    in_Domain: domainIn,
...
// Interconnexions pertinentes entre les 4 marchés suivis par Meridian Power
// (le scope se limite volontairement à ces paires, pas à tous les voisins
// géographiques réels de chaque pays, pour rester gérable).
