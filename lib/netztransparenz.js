...
// physique), 15-min.
// ID AEP: indice intraday du prix de compensation — précurseur avancé du
// reBAP (basé sur les prix intraday plutôt que sur le solde physique),
// 15-min. Même remarque que Hochrechnung: colonne "Datum von" au format ISO
// "YYYY-MM-DD" (vérifié en direct: "2026-07-30;00:00;UTC;..."), pas
// "DD.MM.YYYY" — parseGermanDate produirait un timestamp invalide.
export async function fetchIdAep(dateFrom, dateTo) {
  ...
  return rows
    ...
    .map((r) => ({
      timestamp: parseIsoDate(r["Datum von"], r["(Uhrzeit) von"]),
